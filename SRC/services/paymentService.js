...
export const initializePayment = async (email, amount, reference, metadata = {}, callbackUrl = null) => {
  try {
    ...
    const resp = await axios.post(`${PAYSTACK_BASE}/transaction/initialize`, requestBody, {
      headers: {
        Authorization: `Bearer ${process.env.PAYSTACK_SECRET_KEY}`,
        'Content-Type': 'application/json',
      },
    ...
    if (!data.data || !data.data.authorization_url) {
      throw new Error('Paystack response missing data or authorization_url');
    }

    return data.data;
  } catch (err) {
    // keep error shape similar to previous behaviour
